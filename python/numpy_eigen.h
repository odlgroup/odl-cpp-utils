...
#include <SimRec2DPy/numpy_utils.h>
...
LC        EigenArray out(size.dataRows, size.dataCols);
...
Eigen::Map<EigenArray> mapInput(numeric::array data) {
    ...
    verifySize<EigenArray>(size);
        ...
        throw std::exception(std::string("Array is not pointer compatible with Eigen Array").c_str());
    }
}
